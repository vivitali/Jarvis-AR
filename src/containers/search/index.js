      ...
      <ImageBackground source={Layout.bgImage} style={{width: "100%", height: "100%"}}>
        <View style={styles.container}>
          ...
          {users && users.map((user) =>
            ...
            >
              <View>
                {!!user.carNumber && <Text style={[styles.userDetails, styles.textUnderline]}>{user.carNumber}</Text>}
                {!!user.users_name && <Text style={styles.userDetails}>{user.users_name}</Text>}
              </View>
              <TouchableOpacity
